# -*- coding: utf-8 -*-
"""
Created on Thu Feb 19 16:56:16 2015

@author: Brandon S Coventry       Purdue CAP Lab
Purpose: IC Model for Python: Control program
Revision History: None
Swarm vector
0. Excitatory AMPA Conductance
1. NMDA Conductance
2. GABA Conductance
3. Membrane Potential
4. Ex Input Q10 factor
5. Ex Input Firing rate
6. In input Q10 factor
7. In input Firing rate
8. Difference between ex and in cf (ex fixed)
9. Inhibitory lag
10. Ex psth A1
11. Ex psth B1
12. Ex psth A2
...
for pp in range(ssize):          #Loop through each agent
    ...
    #pdb.set_trace()
    Fitfun = fitfunc(meanvec,sdvec,sgi,expdata["sgirate"][0][0:29],expdata["sgistd"][0][0:29])
    #FTC generation
    ...
    agent[8,4] = cfi
    agent[9,4] = agent[9,0]
    agent[10,4] = agent[10,0]
    agent[11,4] = agent[11,0]
    agent[12,4] = agent[12,0]
    agent[13,4] = agent[13,0]
    agent[14,4] = agent[14,0]
    agent[15,4] = agent[15,0]
    agent[16,4] = agent[16,0]
    agent[17,4] = agent[17,0]
    agent[18,4] = agent[18,0]
    agent[19,4] = agent[19,0]
    agent[20,4] = agent[20,0]
    agent[21,4] = agent[21,0]
    agent[22,4] = agent[22,0]
    agent[23,4] = agent[23,0]
    agent[24,4] = agent[24,0]
    agent[25,4] = agent[25,0]
...
for jj in range(ssize):
    if k <= nummem-1:              #Use this as a "wrap aroudn factor"
        ############################################
        #float->int
        N[k,int(np.mod(jj,neisize))] = jj          #Modulo function takes care of spacing
    else:
        k = 0
        N[k,int(np.mod(jj,neisize))] = jj          #Reset mod factor
    k = k + 1
