	...
	Prop *_prop;
{
 ...
 pnt_receive_size[_mechtype] = 3;
 	...
 	ivoc_help("help ?1 AMPA_LL_IC_OLD /cygdrive/c/Users/labadmin/repos/ICModel/ampaOLD.mod\n");
...
{  double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt;
 ...
 _ltd = ( t - _args[1] ) ;
   _ltd2 = ( t - _args[2] ) ;
   if ( _ltd2 > 0.0 ) {
     A = A + .001 * _args[0] * ( 1.0 - a1 * exp ( - _ltd / tauR ) ) * ( 1.0 - a1 * exp ( - _ltd / tauR ) ) ;
     B = B + .001 * _args[0] * ( 1.0 - a1 * exp ( - _ltd / tauR ) ) * ( 1.0 - a1 * exp ( - _ltd / tauR ) ) ;
     }
   else if ( _ltd > 0.0 ) {
     A = A + .001 * _args[0] * ( 1.0 - a1 * exp ( - _ltd / tauR ) ) ;
     B = B + .001 * _args[0] * ( 1.0 - a1 * exp ( - _ltd / tauR ) ) ;
     }
   else {
     A = A + .001 * _args[0] ;
     B = B + .001 * _args[0] ;
