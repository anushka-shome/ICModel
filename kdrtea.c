...
extern int nrn_get_mechtype();
 ...
 "setdata_kdrtea", _hoc_setdata,
 "settables_kdrtea", _hoc_settables,
 0, 0
};
...
#define usetable usetable_kdrtea
 double usetable = 1;
 ...
 "usetable_kdrtea", &usetable_kdrtea,
 0,0
};
	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 4);
 	...
 	ivoc_help("help ?1 kdrtea /cygdrive/c/Users/labadmin/repos/ICModel/kdrtea.mod\n");
...
static int  _f_settables ( _p, _ppvar, _thread, _nt, _lv ) double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt; 
	double _lv ;
 {
   ninf = 1.0 / ( 1.0 + exp ( ( - 7.2 - _lv ) / 8.9 ) ) ;
   if ( _lv < - 10.0 ) {
     ntau = 0.25 + 4.35 * exp ( ( _lv + 10.0 ) / 10.0 ) ;
     }
   else {
     ntau = 0.25 + 4.35 * exp ( ( - _lv - 10.0 ) / 10.0 ) ;
     }
...
static _initlists(){
 ...
 _slist1[0] = &(n) - _p;  _dlist1[0] = &(Dn) - _p;
   _t_ninf = makevector(642*sizeof(double));
   _t_ntau = makevector(642*sizeof(double));
_first = 0;
}
