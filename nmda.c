	...
	Prop *_prop;
{
 ...
 pnt_receive_size[_mechtype] = 3;
 	...
 	ivoc_help("help ?1 NMDA_LL_IC /cygdrive/c/Users/labadmin/repos/ICModel/nmda.mod\n");
...
{  double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt;
 ...
 _ltd = ( t - _args[1] ) ;
   _ltd2 = ( t - _args[2] ) ;
   _ltd3 = _ltd2 - _ltd ;
   A = A + .001 * _args[0] * ( 1.0 + ( a1 * exp ( - _ltd / tauR ) - a1 * exp ( - _ltd / tauF ) ) ) * ( 1.0 + ( a2 * exp ( - _ltd3 / tauR2 ) - a2 * exp ( - _ltd3 / tauF2 ) ) ) ;
   B = B + .001 * _args[0] * ( 1.0 + ( a1 * exp ( - _ltd / tauR ) - a1 * exp ( - _ltd / tauF ) ) ) * ( 1.0 + ( a2 * exp ( - _ltd3 / tauR2 ) - a2 * exp ( - _ltd3 / tauF2 ) ) ) ;
   _args[2] = _args[1] ;
   _args[1] = t ;
   } }
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   double _lk ;
 _lk = 35.7 * exp ( 2.0 * 0.73 * v * ( .001 ) * F / ( R * ( celsius + 273.16 ) ) ) ;
   g = ( B - A ) ;
   i = .2257 * g * ( 1.0 - ( 1.0 / ( 1.0 + _lk / mgo ) ) ) * ( v - e ) ;
   }
 _current += i;
