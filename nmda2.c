...
#define _ion_ena	*_ppvar[6]._pval
#define _ion_ina	*_ppvar[7]._pval
#define _ion_dinadv	*_ppvar[8]._pval
	...
	Prop *_prop;
{
 ...
 nrn_promote(prop_ion, 0, 1);
 	_ppvar[6]._pval = &prop_ion->param[0]; /* ena */
 	_ppvar[7]._pval = &prop_ion->param[3]; /* ina */
 	_ppvar[8]._pval = &prop_ion->param[4]; /* _ion_dinadv */
 ...
 pnt_receive_size[_mechtype] = 3;
 	...
 	ivoc_help("help ?1 NMDA_LL_IC_2 /cygdrive/c/Users/labadmin/repos/ICModel/nmda2.mod\n");
...
static int _ode_matsol(_NrnThread* _nt, _Memb_list* _ml, int _type) {
 ...
 static void _update_ion_pointer(Datum* _ppvar) {
   ...
   nrn_update_ion_pointer(_na_sym, _ppvar, 6, 0);
   nrn_update_ion_pointer(_na_sym, _ppvar, 7, 3);
   nrn_update_ion_pointer(_na_sym, _ppvar, 8, 4);
 }
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   double _lk ;
 _lk = vdepscale * exp ( 2.0 * 0.73 * v * ( .001 ) * F / ( R * ( celsius + 273.16 ) ) ) ;
   g = ( B - A ) ;
   ica = .2257 * g * fca * ( 1.0 - ( 1.0 / ( 1.0 + _lk / mgo ) ) ) * ( v - eca ) ;
   ina = .2257 * g * fna * ( 1.0 - ( 1.0 / ( 1.0 + _lk / mgo ) ) ) * ( v - ena ) ;
   i = .56 * g * ( 1.0 - fca - fna ) * ( 1.0 - ( 1.0 / ( 1.0 + _lk / mgo ) ) ) * ( v - e ) ;
   }
 _current += ica;
 _current += ina;
 _current += i;
